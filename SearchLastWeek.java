...
public class SearchLastWeek extends common {

	public SearchLastWeek() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Last Week");

			common_searchby("F13", "Last Week", action, driver);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				if (tab_data != "")
					if (date_compare(tab_data, firstDayOfTheWeek) == true) {
						...
						if (i == table_length) {
							logger.log(Status.PASS, "Validation of LastWeek Test verified");
							logger.log(Status.PASS, "Search by LastWeek Test verified");
						}
						if (i == 13) {
							System.out.print("zz " + '\n');
							logger.log(Status.PASS, "Search by LastWeek Test verified");
						...
						logger.log(Status.FAIL,
								"Validation of LastWeek failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by LastWeek Test Failed");
