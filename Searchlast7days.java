...
public class Searchlast7days extends common {

	public Searchlast7days() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Last7days");

			common_searchby("F13", "Last7days", action, driver);

			driver.findElement(By.xpath("//div[contains(@class, 'ui-menu-item-wrapper') and text()='last 7 days']"))
					.click();
			Thread.sleep(1000);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				if (tab_data != "")
					if (date_compare(tab_data, firstDayOfTheWeek) == true) {
						...
						if (i == table_length) {
							logger.log(Status.PASS, "Validation of Last7days Test verified");
							logger.log(Status.PASS, "Search by Last7days Test verified");
						}
						if (i == 13) {
							System.out.print("zz " + '\n');
							logger.log(Status.PASS, "Search by Last7days Test verified");
							save(driver);
							// close_search(search_by, "Last7days", action, driver);
							return;
						}
					} else {
						logger.log(Status.FAIL,
								"Validation of Last7days failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by Last7days Test Failed");
