...
public class SearchbyApproved extends common {

	public SearchbyApproved() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Approved");

			common_searchby("F14", "Approved", action, driver);
			...
			driver.findElement(By.xpath(
					...
					.sendKeys("Completed");
			Thread.sleep(1000);
			driver.findElement(By.xpath("//div[@class='ui-menu-item-wrapper' and text()='Completed']")).click();
			Thread.sleep(1000);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				System.out.print(driver
						.findElement(By.xpath(
								"//*/div/div[4]/div/div/div[1]/div/div[3]/div[2]/div/div/div[" + i + "]/div[4]"))
						.getText().trim());
				...
				if (tab_data.contains("Completed") && date_compare(tab_date, firstDayOfTheWeek) == true) {
					...
					if (i == table_length) {
						logger.log(Status.PASS, "Validation of Approved Test verified");
						logger.log(Status.PASS, "Search by Approved Test verified");
					}
					if (i == 13) {
						logger.log(Status.PASS, "Search by Approved Test verified");
						save(driver);
						// close_search(search_by, "Approved", action, driver);
						return;
					}
				} else {
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by Approved Test Failed");
