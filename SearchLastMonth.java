...
public class SearchLastMonth extends common {

	public SearchLastMonth() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Last Month");

			common_searchby("F13", "Last Month", action, driver);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				if (tab_data != "")
					if (date_compare(tab_data, firstDateOfPreviousMonth) == true) {
						...
						if (i == table_length) {
							logger.log(Status.PASS, "Validation of LastMonth Test verified");
							logger.log(Status.PASS, "Search by LastMonth Test verified");
						}
						if (i == 13) {
							System.out.print("zz " + '\n');
							logger.log(Status.PASS, "Search by LastMonth Test verified");
							save(driver);
							// close_search(search_by, "LastMonth", action, driver);
							return;
						}
					} else {
						logger.log(Status.FAIL,
								"Validation of LastMonth failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by LastMonth Test Failed");
