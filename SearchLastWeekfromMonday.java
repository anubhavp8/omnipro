...
public class SearchLastWeekfromMonday extends common {

	public SearchLastWeekfromMonday() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Last Week from Monday");

			common_searchby("F13", "Last Week from Monday", action, driver);

			driver.findElement(
					By.xpath("//div[contains(@class, 'ui-menu-item-wrapper') and text()='last week (from Monday)']"))
					.click();
			Thread.sleep(1000);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				if (tab_data != "")
					if (date_compare(tab_data, firstDayOfTheWeek) == true) {
						...
						if (i == table_length) {
							logger.log(Status.PASS, "Validation of LastWeekfromMonday Test verified");
							logger.log(Status.PASS, "Search by LastWeekfromMonday Test verified");
						}
						if (i == 13) {
							System.out.print("zz " + '\n');
							logger.log(Status.PASS, "Search by LastWeekfromMonday Test verified");
							save(driver);
							// close_search(search_by, "LastWeekfromMonday", action, driver);
							return;
						}
					} else {
						logger.log(Status.FAIL,
								"Validation of LastWeekfromMonday failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by LastWeekfromMonday Test Failed");
