...
public class SearchToday extends common {

	public SearchToday() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Today");

			common_searchby("F13", "Today", action, driver);

			driver.findElement(By.xpath("//div[contains(@class, 'ui-menu-item-wrapper') and text()='today']")).click();
			Thread.sleep(1000);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				if (tab_data != "")
					if (date_compare(tab_data, strDate) == true) {
						...
						if (i == table_length) {
							logger.log(Status.PASS, "Validation of Today Test verified");
							logger.log(Status.PASS, "Search by Today Test verified");
						}
						if (i == 13) {
							System.out.print("zz " + '\n');
							logger.log(Status.PASS, "Search by Today Test verified");
							save(driver);
							// close_search(search_by, "Today", action, driver);
							return;
						}
					} else {
						logger.log(Status.FAIL,
								"Validation of Today failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by Today Test Failed");
