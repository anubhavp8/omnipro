...
public class SearchbyScore extends common {

	public SearchbyScore() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			Thread.sleep(1000);
			logger = extent.createTest("Search by Score");

			common_searchby("F10", "Score", action, driver);
			...
			for (int i = 1; i <= table_length; i++) {
				...
				if (tab_data.contains(",")) {
					tab_data = tab_data.substring(0, 1);
				}
				
				if (Integer.parseInt(tab_data) <= Integer.parseInt(prop.getProperty("SearchbyScore1")) && date_compare(searchDate, tab_date) == true ) {
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by Score Test Failed");
