...
public class SearchbyTeam extends common {

	public SearchbyTeam() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Team");

			common_searchby("F12", "Team", action, driver);
			...
			Thread.sleep(1000);
			driver.findElement(By.cssSelector(".condition-type > .ui-autocomplete-input")).sendKeys("agent team");
			Thread.sleep(1000);
			...
			Thread.sleep(1000);
			driver.findElement(By.cssSelector(".condition-additional .label-input")).sendKeys(prop.getProperty("AgentTeamName"));
			Thread.sleep(3000);
			...
			for (int i = 1; i <= table_length; i++) {
				String tab_data = driver
						.findElement(By
								.xpath("//*/div/div[4]/div/div/div[1]/div/div[3]/div[2]/div/div/div[" + i + "]/div[10]"))
						.getText().trim();
				...
				if (tab_data.contains(prop.getProperty("AgentTeamName")) && date_compare(tab_date, firstDateOfPreviousMonth) == true ) {
					...
					if (i == table_length) {
						logger.log(Status.PASS, "Validation of Team Test verified");
						logger.log(Status.PASS, "Search by observer Test verified");
					}
					if (i == 13) {
						logger.log(Status.PASS, "Search by Team Test verified");
						save(driver);
						// close_search(search_by, "Team", action, driver);
						return;
					}
				} else {
					logger.log(Status.FAIL, "Validation of Team failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by Team ID Test Failed");
