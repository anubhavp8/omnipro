...
public class SearchbyCampaign extends common {

	public SearchbyCampaign() throws ParseException {
		super();
	...
	public void executetest() throws IOException {
		...
		try {
			...
			logger = extent.createTest("Search by Campaign");

			common_searchby("F12", "Campaign", action, driver);
			...
			Thread.sleep(1000);
			driver.findElement(By.cssSelector(".condition-type > .ui-autocomplete-input")).sendKeys("service");
			Thread.sleep(1000);
			...
			Thread.sleep(1000);
			driver.findElement(By.cssSelector(".condition-additional .label-input")).sendKeys("Voice_Sales");
			Thread.sleep(3000);
			...
			for (int i = 1; i <= table_length; i++) {
				String tab_data = driver
						.findElement(By.xpath(
								"//*/div/div[4]/div/div/div[1]/div/div[3]/div[2]/div/div/div[" + i + "]/div[9]"))
						.getText().trim();
				...
				if (tab_data.contains("Voice_Sales") && date_compare(tab_date, firstDateOfPreviousMonth) == true) {
					...
					if (i == table_length) {
						logger.log(Status.PASS, "Validation of Campaign Test verified");
						logger.log(Status.PASS, "Search by observer Test verified");
					}
					if (i == 13) {
						logger.log(Status.PASS, "Search by Campaign Test verified");
						save(driver);
						// close_search(search_by, "Campaign", action, driver);
						return;
					}
				} else {
					logger.log(Status.FAIL, "Validation of Campaign failed because of " + read_grid_data(driver, i) + " in column " + i);
			...
			e.printStackTrace();
			logger.log(Status.FAIL, "Search by Campaign Test Failed");
